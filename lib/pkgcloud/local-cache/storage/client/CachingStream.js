...
var CachingStream = exports.CachingStream = function(options) {
  ...
  */
  ...
  /**
   * Track if we should destroy when there are no more readers
   * @type {boolean}
   * @private
   */
  this._destroy = false;

  // Detect when the stream has been fully read (written to disk)
...
CachingStream.prototype.pipe = function( dest, pipeOpts ) {
  return this.getReadStream().pipe(dest, pipeOpts);
};
...
CachingStream.prototype.getReadStream = function() {
  if( this._destroy )
    throw new Error('getReadStream() called after destroy()');
  ...
  // Detect the watcher ending and remove it from our collection.
  watcher.on('end', this._handleFileWatcherEnd.bind(this, watcher));

  return watcher;
};

CachingStream.prototype.destroy = function() {
  this._destroy = true;
  this._handleFileWatcherEnd();
};

CachingStream.prototype._handleFileWatcherEnd = function(watcher) {
  var index = this._readers.indexOf(watcher);
  if( index >= 0 )
    this._readers.splice(index,1);
  if( this._destroy && !this._readers.length)
  {
    // Do actual destruction

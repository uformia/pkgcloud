...
Client.prototype.upload = function( options ) {
  ...
  var uploadStream = this._inProgress[container][remote];

  // TODO: Mapping container to different remote container

  uploadStream.pipe(this._backingClient.upload( options )).on('success',
    this._handleBackingSuccess.bind(this, container, remote)
  );
...
Client.prototype.download = function( options ) {
  ...
  if( this._inProgress[container] && this._inProgress[container][remote])
  return this._inProgress[container][remote].getReadStream();
  else
    return this._backingClient.download(options);
};
...
Client.prototype.touchFile = function( options ) {
  ...
  if( !this._inProgress[container][remote] )
  {
    ...
    /* Trigger success on upload completion - Assume streaming to backing will succeed */
...
Client.prototype._handleBackingSuccess = function(container, remote) {
  this._inProgress[container][remote].destroy();
  delete this._inProgress[container][remote];
};